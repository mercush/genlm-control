...
class Potential(ABC, PotentialOperators):
    ...
    async def batch_logp_next_seq(self, context, extensions):
        ...
        scores = await self.batch_score([context + extension for extension in extensions])
    ...
    async def sample(
        ...
        particles = [
            WeightedSample(list(context)) for _ in range(n_samples)
        ]
        n_tokens = 0
    ...
    def make_lazy_weights(self, weights, log=True):
        return LazyWeights(
            weights=weights,
            encode=self.encode_eos,
            decode=self.decode_eos,
            log=log
        )
    ...
    async def assert_properties(self, context, rtol=1e-3, atol=1e-5, top=None, verbosity=0):          
        logp_next = await self.logp_next(context)
        top_logps = logp_next.materialize(top=top)
        ...
        for i, token in enumerate(top_logp_next):
            ...
            diff = {
                'token': repr(token), 'expected': want, 'actual': have,
        ...
        if errors:
            error_msg = "\033[91mPotential properties not satisfied for tokens:\033[0m\n\n"
            for error in errors:
                error_msg += (
